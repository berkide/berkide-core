...
struct TermiosGuard {
    ...
    TermiosGuard() {
        if (tcgetattr(STDIN_FILENO, &old) == 0) {
            termios raw = old;
            cfmakeraw(&raw); // BU SİHİRLİ FONKSİYON TÜM AYARLARI OTomatik YAPAR
            raw.c_cc[VMIN] = 1;
            raw.c_cc[VTIME] = 0;
...
void InputHandler::stop() {
    ...
    // POSIX/Windows: bloklu okuma tek byte bekliyor; kullanıcı bir tuşa basınca çıkacak.
...
#ifndef _WIN32
    TermiosGuard guard; // POSIX: terminali ham moda al
#endif
    KeyEvent ev;
    while (running_) {
...
void InputHandler::dispatch(const KeyEvent& ev) {
    // Önce chord handler (varsa)
    tryDispatchChord(ev);

    // Genel keydown callback
    KeyCallback kd;
    KeyCallback ci;
    {
        std::lock_guard<std::mutex> lock(cbMutex_);
...
std::string InputHandler::toChordString(const KeyEvent& ev) {
    // Türkçe: Karakter girdisi için Ctrl kombinasyonunu tespit edip "Ctrl+S" gibi döndür,
    // özel tuşlar için doğrudan isim döndür ("Enter", "Alt+Left" gibi).
    ...
    if (ev.isChar && ev.ch != U'\0') {
        if (ev.ctrl) out += "Ctrl+";
        if (ev.alt)  out += "Alt+";
        // Tek harf/karakteri chord olarak yaz (örn: Ctrl+S)
...
int InputHandler::readByteBlocking() {
    ...
    return _getch() & 0xFF;
#else
    // Use select() with 100ms timeout so stop() doesn't hang
    fd_set fds;
    FD_ZERO(&fds);
...
static int utf8SeqLen(unsigned char lead) {
    if ((lead & 0x80) == 0)    return 1;  // 0xxxxxxx -> ASCII
...
bool InputHandler::readKeyEvent(KeyEvent& out) {
    ...
    if (c == 0 || c == 224) {
        ...
        switch (k) {
            ...
            default: return true; // bilinmiyorsa yoksayma
        }
    }
    ...
    // ESC => Alt mod veya kaçış dizisi
    if (c == 27) {
        // Bir sonraki byte’a bak
        ...
        if (n1 == '[') {
            ...
            // Bilinmeyen CSI’ları geç
            out.code = KeyCode::Unknown; return true;
        } else { // 'O' prefix: F1..F4 sıklıkla
    ...
    // Enter / Backspace / Tab
    // if (c == '\r' || c == '\n') { out.code = KeyCode::Enter; return true; }
    if (c == 13 || c == 10) {
    out.isChar = false;
    ...
    if (c == 127) { out.code = KeyCode::Backspace; return true; } // DEL
    ...
    if (seqLen > 1) {
        ...
        out.text = mb;
        out.ch = U'\0'; // multi-byte: use text field
    } else {
        out.ch = static_cast<char32_t>(c);
        out.text = utf8FromChar((unsigned char)c);
    }
